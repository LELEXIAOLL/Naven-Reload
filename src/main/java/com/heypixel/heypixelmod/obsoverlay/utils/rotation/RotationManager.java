...
import com.heypixel.heypixelmod.obsoverlay.modules.impl.combat.SilentAim;
...
import com.heypixel.heypixelmod.obsoverlay.modules.impl.render.PearlPrediction;
...
public class RotationManager {
   ...
   public void updateGlobalYaw(EventRunTicks e) {
      if (e.getType() == EventType.PRE && mc.player != null) {
         ...
         } else if (!silentAim.isEnabled() || !silentAim.isWorking()) {  // 修改为使用isWorking()
            active = false;
         } else {
            setRotations(silentAim.getTargetRotation());  // 修改为使用getTargetRotation()
         }
      }
   }
