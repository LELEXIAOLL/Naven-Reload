...
public class InventoryCleaner extends Module {
    ...
    private final FloatValue delay = ValueBuilder.create(this, "Delay (Ticks)")
            ...
            .build()
            .getFloatValue();
    // 修改点1：在副手物品模式中添加"Water Bucket"选项
    ModeValue offhandItems = ValueBuilder.create(this, "Offhand Items")
            .setModes("None", "Golden Apple", "Projectile", "Fishing Rod", "Block", "Water Bucket")
            .build()
            .getModeValue();
    ...
    FloatValue maxArrowSize = ValueBuilder.create(this, "Max Arrow Size")
            ...
            .getFloatValue();
    // 修改点3：调整水桶槽位的可见性逻辑
    BooleanValue switchWaterBucket = ValueBuilder.create(this, "Switch Water Bucket")
            .setDefaultBooleanValue(true)
            .setVisibility(() -> !this.offhandItems.isCurrentMode("Water Bucket")) // 当副手是水桶时隐藏
            .build()
            .getBooleanValue();
    FloatValue waterBucketSlot = ValueBuilder.create(this, "Water Bucket Slot")
            ...
            .setMinFloatValue(1.0F)
            .setMaxFloatValue(9.0F)
            .setVisibility(() -> this.switchWaterBucket.getCurrentValue() && !this.offhandItems.isCurrentMode("Water Bucket"))
            .build()
            .getFloatValue();
    ...
    FloatValue fireballSlot = ValueBuilder.create(this, "Fireball & TNT Slot")
    ...
    FloatValue rodSlot = ValueBuilder.create(this, "Rod Slot")
            ...
            .build()
            .getFloatValue();
    BooleanValue keepFishingRod = ValueBuilder.create(this, "Keep Fishing Rod").setDefaultBooleanValue(false).build().getBooleanValue();

    BooleanValue keepCobweb = ValueBuilder.create(this, "Keep Cobweb").setDefaultBooleanValue(false).build().getBooleanValue();

    FloatValue cobwebSlot = ValueBuilder.create(this, "Cobweb Slot")
            .setDefaultFloatValue(9.0F)
            .setFloatStep(1.0F)
            .setMinFloatValue(1.0F)
            .setMaxFloatValue(9.0F)
            .setVisibility(() -> this.keepCobweb.getCurrentValue()) // 仅在保留蜘蛛网时显示
            .build()
            .getFloatValue();
    int noMoveTicks = 0;
    ...
    public static boolean shouldKeepCobweb() {
        return ((InventoryCleaner)Naven.getInstance().getModuleManager().getModule(InventoryCleaner.class)).keepCobweb.getCurrentValue();
    }

    public static boolean shouldKeepFishingRod() {
        return ((InventoryCleaner)Naven.getInstance().getModuleManager().getModule(InventoryCleaner.class)).keepFishingRod.getCurrentValue();
    }
    ...
    public boolean isItemUseful(ItemStack stack) {
        if (this.keepCobweb.getCurrentValue() && stack.getItem() == Items.COBWEB) {
            return true; // 如果是蜘蛛网且开关打开，则视为有用
        }
        if (this.keepFishingRod.getCurrentValue() && stack.getItem() instanceof FishingRodItem) {
            return true; // 如果是鱼竿且开关打开，则视为有用
        }
        if (stack.isEmpty()) {
            return false;
        } else if (InventoryUtils.isGodItem(stack)) {
            return true;
        } else if (stack.getItem() instanceof FishingRodItem && !shouldKeepFishingRod() && InventoryUtils.getItemCount(Items.FISHING_ROD) > 1) {
            return false;
        } else if (stack.getItem() == Items.COBWEB && !shouldKeepCobweb()) {
                return false;
            ...
            return false;
        } else if (stack.getItem() == Items.ANVIL) {
            return false;
    ...
    public void onMotion(EventMotion e) {
        if (e.getType() == EventType.PRE) {
            ...
            if (this.switchFireball.getCurrentValue()) {
                int fireballSlotIndex = (int)(this.fireballSlot.getCurrentValue() - 1.0F);
                ItemStack currentStack = mc.player.getInventory().items.get(fireballSlotIndex);

                // 检查当前槽位是否是火球或TNT
                boolean isFireball = currentStack.getItem() == Items.FIRE_CHARGE;
                boolean isTNT = currentStack.getItem() == Items.TNT;

                // 优先放置火球
                if (InventoryUtils.getItemCount(Items.FIRE_CHARGE) > 0) {
                    if (!isFireball) {
                        this.swapItem(fireballSlotIndex, Items.FIRE_CHARGE);
                    }
                }
                // 如果没有火球但有TNT，放置TNT
                else if (InventoryUtils.getItemCount(Items.TNT) > 0) {
                    if (!isTNT) {
                        this.swapItem(fireballSlotIndex, Items.TNT);
                    }
                }
            }
            ...
            } else if (this.offhandItems.isCurrentMode("Water Bucket")) {
                ItemStack offHand = mc.player.getInventory().offhand.get(0);
                int slot = InventoryUtils.getItemSlot(Items.WATER_BUCKET);
                if (slot != -1 && timer.delay(this.delay.getCurrentValue())) {
            ...
            // 当副手不是水桶时才执行快捷栏切换
            if (this.switchWaterBucket.getCurrentValue() && !this.offhandItems.isCurrentMode("Water Bucket")) {
                this.swapItem((int)(this.waterBucketSlot.getCurrentValue() - 1.0F), Items.WATER_BUCKET);
            }
            if (this.keepCobweb.getCurrentValue()) {
                this.swapItem((int)(this.cobwebSlot.getCurrentValue() - 1.0F), Items.COBWEB);
            }
