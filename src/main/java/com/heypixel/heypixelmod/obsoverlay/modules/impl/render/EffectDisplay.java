...
public class EffectDisplay extends Module {
   ...
   public void onRender(EventRender2D e) {
      HUDEditor.HUDElement effectsElement = HUDEditor.getInstance().getHUDElement("potioneffects");
      ...
      int startY = (int) effectsElement.y;
      ...
      for (Entry<MobEffect, MobEffectInfo> entry : this.infos.entrySet()) {
         ...
         if (!effectInfo.shouldDisappear) {
            if (effectInfo.width > maxWidth) {
               maxWidth = effectInfo.width;
            }
         }

         if (effectInfo.shouldDisappear) {
            effectInfo.xTimer.target = -effectInfo.width - 20.0F;
            if (effectInfo.xTimer.value <= -effectInfo.width - 19.0F) {
            ...
            effectInfo.xTimer.target = (float) effectsElement.x;
      ...
      effectsElement.width = maxWidth;
      effectsElement.height = totalHeight;
   }
