...
public class LongJump extends Module {
   public ModeValue renderStyles = ValueBuilder.create(this, "RenderStyles")
           .setDefaultModeIndex(0)
           .setModes("Simple", "LingDong")
           .build()
           .getModeValue();
   ...
   public void onEnable() {
      ...
      ChatUtils.addChatMessage("§aLongJump enabled! Press Z to jump & use fireball, C to release each knockback");
   }
   ...
   public void onUpdate(EventUpdate event) {
      if (this.isEnabled()) {
         if (this.shouldDisableAndRelease) {
            ...
            boolean currentZ = GLFW.glfwGetKey(mc.getWindow().getWindow(), GLFW.GLFW_KEY_Z) == GLFW.GLFW_PRESS;
            if (currentZ && !this.zPressed) {
               this.zPressed = true;
            ...
            boolean currentC = GLFW.glfwGetKey(mc.getWindow().getWindow(), GLFW.GLFW_KEY_C) == GLFW.GLFW_PRESS;
            if (currentC && !this.cPressed) {
               this.cPressed = true;
   ...
   public void onRender2D(EventRender2D event) {
      // [修改] 只有在 Simple 模式下才渲染旧的UI
      if (this.isEnabled() && renderStyles.isCurrentMode("Simple")) {
         int screenWidth = mc.getWindow().getGuiScaledWidth();
         int screenHeight = mc.getWindow().getGuiScaledHeight();
         String statusText;
         if (this.delayed) {
            statusText = String.format("§eIntercepting: %d packets | Time: %.1fs | Press C (%d/%d)", packets.size(), (float)(System.currentTimeMillis() - delayStartTime) / 1000.0F, releasedKnockbacks, receivedKnockbacks);
         } else if (this.isUsingItem) {
            statusText = "§aUsing fireball #" + this.usedFireballCount;
         } else {
            statusText = "§bWaiting for input | Z: Jump & use fireball | C: Release";
         }
      ...
      if (this.isEnabled() && mc.level != null) {
         if (this.delayed && event.getType() == EventType.RECEIVE) {
            ...
            if (event.getPacket() instanceof ClientboundSetEntityMotionPacket packet
               ...
               mc.execute(() -> ChatUtils.addChatMessage("§ePacket interception started, press C to release each"));
            }
         }
      } else {
         if (this.delayed) {
   ...
   public float getTimeElapsed() {
      if (!delayed) return 0.0f;
      return (System.currentTimeMillis() - delayStartTime) / 1000.0f;
   }

   public int getCurrentKnockbacks() {
      return releasedKnockbacks;
   }

   public int getTotalKnockbacks() {
      return receivedKnockbacks;
   }

   public int getUsedFireballs() {
      return usedFireballCount;
   }
}