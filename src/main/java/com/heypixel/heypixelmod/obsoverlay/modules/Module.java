   ...
   public void setEnabled(boolean enabled) {
      try {
         ...
         if (enabled) {
            ...
            if (!(this instanceof ClickGUIModule) && !NotificationManager.isConfigLoading) {
               HUD module = (HUD) Naven.getInstance().getModuleManager().getModule(HUD.class);
               // 增加判空防止极其罕见的空指针
               if (module != null && module.moduleToggleSound.getCurrentValue()) {
                  if (mc.player != null) {
                     mc.player.playSound(SoundEvents.WOODEN_BUTTON_CLICK_ON, 0.5F, 1.3F);
                  }
               }
               Notification notification = new Notification(NotificationLevel.SUCCESS, this.name + " Enabled!", 2000L);
            ...
            if (!(this instanceof ClickGUIModule) && !NotificationManager.isConfigLoading) {
               ...
               Notification notification = new Notification(NotificationLevel.ERROR, this.name + " Disabled!", 2000L);
               naven.getNotificationManager().addNotification(notification);
            }
         }
